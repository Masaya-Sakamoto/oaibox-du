...
/* C-RNTI range (0001-FFF2) (TS 38.321 Table 7.1-1) */
#define NR_C_RNTI_MIN 0x0001
#define NR_C_RNTI_MAX 0xfff2

/* 5.3.3.3 TS 38.331: Random UE identity mass for 39-bit values */
                                                 ...
                                                 const f1ap_initial_ul_rrc_message_t *msg)
{
  ...
  if (old_rnti < NR_C_RNTI_MIN || old_rnti > NR_C_RNTI_MAX) {
    LOG_E(NR_RRC, "C-RNTI %04x out of range (0x0001-0xFFF2): rejecting RRCReestablishmentRequest\n", old_rnti);
    return;
  }
  ...
  if (!UE->Srb[SRB2].Active) {
    LOG_E(NR_RRC, "UE context not valid for re-establishment (no SRB2), fallback to RRC setup\n");
    ngap_cause = NGAP_CAUSE_RADIO_NETWORK_RELEASE_DUE_TO_NGRAN_GENERATED_REASON;
    goto fallback_rrc_setup;
  }

  if (seq_arr_size(&UE->drbs) == 0) {
    LOG_E(NR_RRC, "UE context not valid for re-establishment (no DRB), fallback to RRC setup\n");
    ngap_cause = NGAP_CAUSE_RADIO_NETWORK_RELEASE_DUE_TO_NGRAN_GENERATED_REASON;
    goto fallback_rrc_setup;
  }
                                                   ...
                                                   NR_MeasurementReport_t *measurementReport)
{
  ...
  switch (event_triggered->eventId.present) {
    case NR_EventTriggerConfig__eventId_PR_eventA2:
      ...
      // Implement a criterion to select the neighboring cell
      nr_neighbour_cell_t *target_neighbour = NULL;
      const neighbour_cell_configuration_t *neighbour_config = get_neighbour_cell_config(rrc, source_cell->info.cell_id);
      const seq_arr_t *neighbour_cells = neighbour_config ? &neighbour_config->neighbour_cells : NULL;
      FOR_EACH_SEQ_ARR (nr_neighbour_cell_t *, neighbour, neighbour_cells) {
        if (scell_pci != neighbour->physicalCellId)
      ...
      const nr_rrc_cell_container_t *target_cell = get_cell_by_cell_id(&rrc->cells, target_neighbour->nrcell_id);
