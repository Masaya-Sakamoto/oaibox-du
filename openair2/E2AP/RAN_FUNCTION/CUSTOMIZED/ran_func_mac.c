...
bool read_mac_sm(void* data)
{
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    ...
    struct NR_PLMN_IdentityInfo *plmn_IdentityInfo =
        nrmac->common_channels[0]
            .sib1->message.choice.c1->choice.systemInformationBlockType1->cellAccessRelatedInfo.plmn_IdentityInfoList.list.array[0];
    rd->nr_cellid  = BIT_STRING_to_uint64(&plmn_IdentityInfo->cellIdentity);
    ...
    if (is_dl_slot(rd->slot, &nrmac->frame_structure)) {
      rd->dl_curr_tbs = UE->mac_stats.dl.current_bytes;
      rd->dl_sched_rb = UE->mac_stats.dl.current_rbs;
    }
    if (is_ul_slot(rd->slot, &nrmac->frame_structure)) {
    ...
    rd->pusch_snr = nr_mac_get_snr(&sched_ctrl->pusch_pc);
    rd->pucch_snr = nr_mac_get_snr(&sched_ctrl->pucch_pc);
    ...
    rd->raw_rssi = -128.0 + sched_ctrl->pusch_pc.avg_rssi * 0.1;
    uint8_t cqi = (sched_ctrl->pucch_pc.avg_snr + 640) / 5.0;
    if (cqi > 15) {
      cqi = 15;
    }
    rd->cqi = cqi;
    if (sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.wb_cqi_1tb != 0) {
      rd->cqi = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.wb_cqi_1tb;
    }
    rd->rsrp = UE->mac_stats.num_rsrp_meas > 0 ? UE->mac_stats.cumul_rsrp / UE->mac_stats.num_rsrp_meas : 0;
    rd->dl_qm = nr_get_Qm_dl(sched_ctrl->dl_bler_stats.mcs, UE->current_DL_BWP.mcsTableIdx);
    rd->ul_qm = nr_get_Qm_ul(sched_ctrl->ul_bler_stats.mcs, UE->current_UL_BWP.mcs_table);
