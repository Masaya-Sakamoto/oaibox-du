...
void read_mac_setup_sm(void* data)
{
  ...
  int sliceId = mac_ctrl_req->msg.sliceId;
  mac->nssai_config_dl.active[sliceId] = mac_ctrl_req->msg.active;
  mac->nssai_config_dl.agg_bitrate[sliceId] = mac_ctrl_req->msg.dl_agg_bitrate;
  mac->nssai_config_ul.active[sliceId] = mac_ctrl_req->msg.active;
  mac->nssai_config_ul.agg_bitrate[sliceId] = mac_ctrl_req->msg.ul_agg_bitrate;
...
sm_ag_if_ans_t write_ctrl_mac_sm(void const* data)
{
  ...
  // sliceId 0 not defined in standard
  if (mac_ctrl_req->msg.sliceId > 0) {
    gNB_MAC_INST* mac = RC.nrmac[mod_id];
  ...
  if (mac_ctrl_req->msg.ue_id > 0) {
    LOG_I(NR_MAC,"MAC SM triggering BWP switching for UE ID %d with dl_bwp_id %d and ul_bwp_id %d\n", mac_ctrl_req->msg.ue_id, mac_ctrl_req->msg.dl_bwp_id, mac_ctrl_req->msg.ul_bwp_id);
  	nr_bwp_switching(mod_id, mac_ctrl_req->msg.ue_id, mac_ctrl_req->msg.dl_bwp_id, mac_ctrl_req->msg.ul_bwp_id);
  }
