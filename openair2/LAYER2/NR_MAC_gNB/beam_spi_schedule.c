 ...
 * a beam_spi_slot_batch_t via a registered callback.
 *
 * When no callback is registered (i.e. no hardware backend), this is a no-op.
 */
...
/* ── Callback function pointer (set by SPI worker at startup) ────────── */

static beam_spi_enqueue_fn_t g_beam_spi_enqueue_fn = NULL;

void beam_spi_set_enqueue_fn(beam_spi_enqueue_fn_t fn)
{
    g_beam_spi_enqueue_fn = fn;
}

/* ── Helper: determine TX/RX direction from TDD slot structure ───────── */
    ...
    switch (bm->slot_type) {
        ...
        case TDD_NR_MIXED_SLOT:
            if (symbol < bm->num_dl_symbols)
                ...
                return BEAM_SPI_TX;  /* guard symbols default to TX */
        default:
            return BEAM_SPI_TX;
    }
}

/* ── Helper: beam_allocation slot index (same as gNB_scheduler_primitives.c) ── */
...
void beam_spi_schedule_slot(gNB_MAC_INST *gNB, frame_t frame, slot_t slot)
{
    ...
    if (batch->n_entries > 0) {
        if (!g_beam_spi_enqueue_fn(batch)) {
