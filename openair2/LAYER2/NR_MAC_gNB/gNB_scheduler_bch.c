...
static uint32_t get_tbs_bch(NR_sched_pdsch_t *pdsch, uint32_t num_total_bytes, uint16_t *vrb_map)
{
                                      ...
                                      uint16_t num_total_bytes)
{
  ...
  uint32_t TBS = get_tbs_bch(&gNB_mac->sched_ctrlCommon->sched_pdsch, gNB_mac->sched_ctrlCommon->num_total_bytes, vrb_map);
  if (TBS == 0)
    return 0;
  ...
  for (int i = 0; i < L_max; i++) {
    ...
    if((frameP % 2 == type0_PDCCH_CSS_config->sfn_c) &&
      ...
      for (int t = 0; t < 16; t++) {
        ...
        // the UE may assume that this PDSCH resource allocation is not applied (Table 5.1.2.1.1-4 and 5)
        if (table_type == defaultB && (t == 12 || t == 13 || t == 14))
          continue;
        if (table_type == defaultC && (t == 1 || t == 6 || t == 13 || t == 14 || t == 15 || t == 16))
          continue;
        // assuming normal CP
        ...
        TBS = schedule_control_sib1(gNB_mac,
                                    CC_id,
                                    &sched_pdcch,
                                    type0_PDCCH_CSS_config,
                                    t,
                                    ...
                                    int payload_idx)
{
  ...
  uint32_t TBS = get_tbs_bch(&sched_pdsch_otherSI, num_total_bytes, vrb_map);
  AssertFatal(TBS > 0, "Couldn't allocate TBS for other SIB\n");
  sched_pdsch_otherSI.tb_size = TBS;
