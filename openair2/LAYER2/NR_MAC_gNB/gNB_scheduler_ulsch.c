...
nssai_bwp_info_t get_pusch_nssai_start_size(gNB_MAC_INST *nrmac, NR_UE_info_t *UE)
{
  ...
  nssai_bwp_info.prb_stop = ul_bwp->BWPSize - 1;
                                               ...
                                               const NR_tda_info_t *tda_info)
{
  ...
  if (reuse_old_tda && nrOfLayers == retInfo->nrOfLayers) {
    ...
    if (beam.idx < 0)
      return ret;
    rballoc_mask = &nrmac->common_channels[CC_id].vrb_map_UL[beam.idx][buf_idx * MAX_BWP_SIZE];
  ...
  for (int rb = 0; rb < new_sched.rbSize; rb++)
    rballoc_mask[bwpStart + new_sched.rbStart + rb] |= SL_to_bitmap(new_sched.tda_info.startSymbolIndex, new_sched.tda_info.nrOfSymbols);
  return ret;
}
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  while (iterator->UE != NULL) {
    ...
    if (rbStart + min_rb > rbStop || available_rb < min_rb) {
      reset_beam_status(&nrmac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, dci_beam.new_beam);
      reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
